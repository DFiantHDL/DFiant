package dfhdl
import dfhdl.DFDesign.Control.Op
import dfhdl.DFDesign.DB.Patch
import dfhdl.compiler.{DependencyContext, SanityCheck}
import compiler.csprinter.CSPrinter
import dfhdl.internals._

import scala.annotation.{nowarn, tailrec}
import scala.collection.{immutable, mutable}
import dfhdl.sim.DFSimDesign
...
abstract class DFDesign(implicit ctx : DFDesign.Context) extends DFDesign.Abstract {
  private[dfhdl] final lazy val __ctx : DFDesign.Context = ctx
  private[dfhdl] final lazy val __portArgs : Map[DFAny, DFAny] = {
...
object DFDesign {
  protected[dfhdl] type Context = DFBlock.Context

  trait Abstract extends DFOwner.Container {
    type Owner = DFDesign.Block
    private[dfhdl] lazy val inlinedRep : Option[DFInlineComponent.Rep] = None
    private[dfhdl] lazy val simMode : DFSimDesign.Mode = DFSimDesign.Mode.Off
    private[dfhdl] val __ctx : DFDesign.Context
    protected[dfhdl] final implicit lazy val __db : DFDesign.DB.Mutable = __ctx.db
    private lazy val designType : String = typeName.split("\\.").last
    private[dfhdl] final val owner : DFDesign.Block = DFDesign.Block.Internal(this)(designType, inlinedRep, simMode)(__ctx)
    protected[dfhdl] final implicit val __dir : DFDir = ASIS
  ...
  /**
    * The entire standard DFHDL syntax and implicit conversion required.
...
//                println("untrack", fsm)
      ...
      protected[dfhdl] object OwnershipContext {
