...
abstract class DFInlineComponent[Type <: DFAny.Type](dfType : Type)(
  ...
  override private[dfhdl] lazy val inlinedRep : Option[DFInlineComponent.Rep] = Some(rep)
...
object DFInlineComponent {
  trait Rep extends Product with Serializable {
    protected[dfhdl] def =~(that : DFInlineComponent.Rep)(implicit getSet : MemberGetSet) : Boolean
...
final case class EdgeDetect(bit : Bit, edge : EdgeDetect.Edge)(
  implicit ctx : ContextOf[EdgeDetect]
) extends DFInlineComponent[Bool.Type](Bool.Type(logical = true)) {
  lazy val rep : DFInlineComponent.Rep = EdgeDetect.Rep(bit, edge)
  private val boolIn = Bit <> IN
  edge match {
...
object EdgeDetect {
  ...
  final case class Rep(bitRef : DFInlineComponent.Ref, edge : EdgeDetect.Edge) extends DFInlineComponent.Rep {
    protected[dfhdl] def =~(that : DFInlineComponent.Rep)(implicit getSet : MemberGetSet) : Boolean = that match {
