...
class DFScope(val defMeta : Option[Meta] = None, val nameFlatten : DFOwner.NameFlatten = DFOwner.NameFlatten.UnderscoreSuffix)(implicit ctx : DFBlock.Context) extends DFOwner.Container {
  ...
  private[dfhdl] final val __ctx : DFBlock.Context = ctx
  private[dfhdl] final val owner : Owner = DFScope.Owner(this)(defMeta, nameFlatten)(ctx)
  protected[dfhdl] final implicit val __dir : DFDir = ctx.dir
  protected[dfhdl] final implicit lazy val __db : DFDesign.DB.Mutable = ctx.db
