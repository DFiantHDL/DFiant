...
trait DFMember extends HasTypeName with Product with Serializable {self =>
  ...
  final private[dfhdl] def getOwnerChain(implicit getSet : MemberGetSet) : List[DFBlock] =
  ...
  protected[dfhdl] def =~(that : DFMember)(implicit getSet : MemberGetSet) : Boolean
...
object MemberGetSet {
  import dfhdl.compiler.printer.Printer
