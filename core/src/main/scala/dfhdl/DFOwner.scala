...
object DFOwner {
  trait Container extends HasTypeName with DFDesign.Frontend with DisallowExternalExtensions {
    type Owner <: DFOwner
    private[dfhdl] val owner : Owner
    protected[dfhdl] val __db: DFDesign.DB.Mutable
    private[dfhdl] val __ctx : DFMember.Context
    ...
    final private[dfhdl] lazy val __parent : Container = if (__ctx.container == null) this else __ctx.container
    ...
    final private[dfhdl] def isInsideParent(that : Container) : Boolean = isInsideParent(this, that)
    protected[dfhdl] def onEnterContainer() : Unit = {}
    protected[dfhdl] def onExitContainer() : Unit = {}
    protected[dfhdl] def onCreateContainer() : Unit = {}
