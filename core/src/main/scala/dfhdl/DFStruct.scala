...
import dfhdl.DFAny.{UninitializedDcl, `Op==,!=`}
import dfhdl.DFStruct.Fields
...
object DFStruct extends DFAny.Companion {
  abstract class Fields(implicit meta : Meta) extends DFAny.Token.Frontend with HasWidth { fields =>
    ...
    private[dfhdl] final def name(implicit getSet: MemberGetSet) : String = getSet.getGlobalTag[DFMember.NameTag](this) match {
  ...
  object Fields {
    ...
    object WidthOf {
      ...
      def evMacro[F <: Fields : c.WeakTypeTag](c : whitebox.Context) : c.Tree = {
        ...
        q"new dfhdl.DFStruct.Fields.WidthOf[$tpe]{type Out = $widthTpe}"
      }
    }
  ...
  final case class Field[Type <: DFAny.Type](dfType : Type)(
    ...
    private[dfhdl] val fieldName : String = meta.name
  ...
  object DFFields {
    def applyMacro[F <: Fields : c.WeakTypeTag, Mod <: DFAny.Modifier : c.WeakTypeTag](c : whitebox.Context) : c.Tree = {
      ...
      val defFields = publicFields.map{d =>
        ...
        val selectorTree = q"dfhdl.DFStruct.Selector($dfTypeTree, struct, ${d.name.toString})(ctx)"
        q"def ${d.name.toTermName}(implicit ctx : dfhdl.DFAny.Context) = $selectorTree"
      }
      val genTree =
        q"""
            new dfhdl.DFStruct.DFFields($structTree){
              ..$defFields
            }
         """
//      println(genTree)
