...
import dfhdl.DFDecimal.{`LF == RF`, `LF >= RF`, `LS == RS`, `LS signMatch RS`, `VarF >= ConstF`, `VarS signMatch ConstS`, `VarW >= ConstW`}
import dfhdl.internals.BitsWidthOf.SignedCfg.IntAux
...
object DFDecimal extends DFAny.Companion {
  ...
  object Token {
    ...
    def decImplStringInterpolator(c: whitebox.Context) : c.Tree = {
      ...
      val buildTree = q"dfhdl.DFDecimal.Token.fromDecString($fullExpressionTree).toOption.get"
      q"""
         new dfhdl.Interpolator[dfhdl.DFDecimal.Token, "d"] {
           type Out = dfhdl.DFDecimal.TokenW[$signTpe, $widthTpe, $fractionWidthTpe]
