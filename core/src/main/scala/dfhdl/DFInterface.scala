...
abstract class DFInterface(
  ...
  private[dfhdl] final lazy val __ctx : DFInterface.Context = ctx
  private[dfhdl] final def updateDir(updatedDir : DFDir) : this.type = {
  ...
  protected[dfhdl] object DEFAULT_DIR extends DEFAULT_DIR
}
...
abstract class DFCrossInterface(
  ...
  protected[dfhdl] def crossConnection(twin : this.type)(implicit ctx : DFBlock.Context) : Unit
}
...
object DFInterface {
  trait Abstract extends DFOwner.Container {
    ...
    private[dfhdl] val __ctx : DFInterface.Context
    val nameFlatten : DFOwner.NameFlatten
    private[dfhdl] final val owner : Owner = DFInterface.Owner(this)(nameFlatten)(__ctx)
    protected[dfhdl] final implicit val __dir : DFDir = __ctx.dir
  ...
  implicit class InterfaceExt[T <: DFInterface](t : T) {
    ...
    private[dfhdl] def replaceOwnerAndMembers(from : DFOwner, to : DFOwner)(implicit getSet: MemberGetSet) : Unit = {
