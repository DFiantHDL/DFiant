...
abstract class TagsOf[-D <: DFDesign] extends DisallowExternalExtensions {
  ...
  private[dfhdl] def attachDesign(dsn : => D) : this.type = {
    _dsn = () => dsn
    this
  }
  private[dfhdl] var taggedMembers : List[TaggedMember[DFMember]] = Nil
  ...
  private[dfhdl] def tagMember[M <: DFMember, CT <: DFMember.CustomTagOf[M] : ClassTag](
    member : => M, tag : CT
  ) : TaggedMember[M] = {
