...
object DFEnum extends DFAny.Companion {
  ...
  sealed abstract class Entries(implicit meta : Meta) {
    private[dfhdl] var all : immutable.ListMap[BigInt, Entries.Entry] = immutable.ListMap.empty[BigInt, Entries.Entry]
    private[dfhdl] def update(entry : Entries.Entry) : Unit = {
  ...
  abstract class Auto[E <: Encoding](val encoding : E = Encoding.Default)(
    ...
    class Entry private[dfhdl] (implicit val entries : Entries, private[dfhdl] val meta : Meta) extends Entries.Entry {
  ...
  abstract class Manual[Width <: Int with Singleton](val width : TwoFace.Int[Width])(
    ...
    class Entry private[dfhdl] (val value : BigInt, val entries : Entries)(implicit private[dfhdl] val meta : Meta) extends Entries.Entry {
      entries.update(this)
