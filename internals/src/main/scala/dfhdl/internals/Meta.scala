...
import dfhdl.internals.Meta.Name.Anonymous
...
//      val name = getOwnerName(c)(owner)
//      c.Expr[Meta.Name](q"""dfhdl.internals.Meta.Name($name)""")
//    }
  ...
  object SymbolOf {
    ...
    def evMacro[T](c: whitebox.Context)(implicit t : c.WeakTypeTag[T]): c.Tree = {
      ...
      q"""dfhdl.internals.Meta.SymbolOf[$tpe]($fullName)"""
    }
  }
  ...
  def evMacro(c: whitebox.Context)(lateConstructionConfig : c.Tree): c.Tree = {
    ...
    val anonymous =
      if (enclosingOwnerName.contains("<local") || enclosingOwnerName == "$anonfun") q"dfhdl.internals.Meta.Name.Anonymous.On"
      else q"dfhdl.internals.Meta.Name.Anonymous.Off"
//    println(name, anonymous, owner.isMethod, c.enclosingPosition)
    q"""dfhdl.internals.Meta(dfhdl.internals.Meta.Name($name, $anonymous), dfhdl.internals.Meta.Position($file, $line, $column), dfhdl.internals.Meta.Position($nameFile, $nameLine, $nameColumn), $lateConstructionConfig($lateConstruction))"""
  }
