...
@df class loopback extends DFDesign {
  ...
  private val i = Bits(31) <> VAR //i.prev
  private val i_plus1 = Bits(31) <> VAR //i+1
  private val i_plus1_reg = Bits(31) <> VAR //(i+1).prev
  private val d_addr_read_reg = Bits(32) <> VAR
  private val notDataEnd = Bit <> VAR
...
@df class LoopbackDriver extends DFSimDesign {
  ...
  private val read_flag = Bool <> VAR init false
  private val read_first = Bool <> VAR init true
  ...
  def dataFunc(cnt : UInt[32])(implicit ctx : DFBlock.Context) : Bits[32] = {
    cnt.bits
  }
  private val read_cnt = UInt(32) <> VAR init 0
  private val read_size = UInt(32) <> VAR
  ...
  private val write_cnt = UInt(32) <> VAR init 0
  private val write_size = UInt(32) <> VAR
  ...
  private val write_flag = Bool <> VAR init false
  private val write_first = Bool <> VAR init true
