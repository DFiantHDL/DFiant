...
import dfhdl.internals.targetName
...
package object util {
  ...
  final implicit class __BitExpand(bit : Bit.type) {
  ...
  final implicit class __ActiveHighOps(bit : ActiveHigh.TVal) {
    @df def isActive : Bool = bit.actual
    @targetName("not")
  ...
  @df final implicit def __BitActiveHigh_ac_BitActiveLow : DFAny.`Op:=,<>`.Builder[ActiveHigh.TType, ActiveLow.TVal] = (left, right) => {
    !right
  }
  @df final implicit def __BitActiveLow_ac_BitActiveHigh : DFAny.`Op:=,<>`.Builder[ActiveLow.TType, ActiveHigh.TVal] = (left, right) => {
    !right
  }
  @df final implicit def __BitActiveHigh_TokenStatus[V <: Status]
  ...
  @df final implicit def __BitActiveLow_TokenStatus[V <: Status]
